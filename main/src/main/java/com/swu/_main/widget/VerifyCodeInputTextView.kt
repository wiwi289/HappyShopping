...
class VerifyCodeInputTextView @JvmOverloads constructor(
    ...
    private val sentVerifyTextColor = Color.parseColor("#EE7B4D")
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        if (mVerifyRect.contains(point.x,point.y) && verifyEnabled && !boolLock) {
            // 锁住当前点击事件
            boolLock = true
            //位于点击区域，发送验证码
            sendVerifyCode()
            postDelayed( { boolLock = false }, 100)
            return true
        }
